...
/**
 * Describes super push command behavior for the Comet subsystem.
 */
public class SuperPushCommand extends ChatCommand {
    ...
    private void push(RoomEntity entity, Session client) {
        ...
        if (!((Math.abs((posX - playerX)) >= 3) || (Math.abs(posY - playerY) >= 3))) {
            switch (rot) {
                case 4:
                    posY += 3;
                    break;
    ...
    public String getDescription() {
        return Locale.get("command.superpush.description");
    }
