...
/**
 * Describes remove blur floor item behavior for the room subsystem.
 */
public class RemoveBlurFloorItem extends RoomItemFloor {
    private RoomEntity vendingEntity;
    private int state = -1;

    /**
     * Creates a remove blur floor item instance for the room subsystem.
     *
     * @param itemData Item data supplied by the caller.
     * @param room Room participating in the operation.
     */
    public RemoveBlurFloorItem(RoomItemData itemData, Room room) {
        super(itemData, room);
    }
    ...
    public boolean onInteract(RoomEntity entity, int requestData, boolean isWiredTrigger) {
        ...
        WebSocketSessionManager.getInstance().sendMessage(p.getPlayer().getSession().getWsChannel(), new BlurEffectWebPacket("smoke_stop"));
        this.getRoom().getEntities().broadcastMessage(new TalkMessageComposer(p.getId(), "<i>*parece ver con más claridad las cosas*</i>", ChatEmotion.SHOCKED, 5));
