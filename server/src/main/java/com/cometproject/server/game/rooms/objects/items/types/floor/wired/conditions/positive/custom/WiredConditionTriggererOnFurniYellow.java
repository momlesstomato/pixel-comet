...
/**
 * Describes wired condition triggerer on furni yellow behavior for the room subsystem.
 */
public class WiredConditionTriggererOnFurniYellow extends WiredConditionItem {
    /**
     * Creates a wired condition triggerer on furni yellow instance for the room subsystem.
     *
    ...
    public boolean evaluate(RoomEntity entity, Object data) {
        ...
        block0: for (long itemId : this.getWiredData().getSelectedIds()) {
            for (RoomItemFloor itemOnEntity : entity.getRoom().getItems().getItemsOnSquare(entity.getPosition().getX(), entity.getPosition().getY())) {
                if (itemOnEntity.getId() != itemId || !playerEntity.getGameTeam().equals(GameTeam.YELLOW)) continue;
                isOnFurni = true;
                continue block0;
            }
        }
