...
/**
 * Describes wired custom teleport red behavior for the room subsystem.
 */
public class WiredCustomTeleportRed extends WiredActionItem {

    /**
     * Creates a wired custom teleport red instance for the room subsystem.
     *
    ...
    public void onEventComplete(WiredItemEvent event) {
        ...
        for (long itemId : this.getWiredData().getSelectedIds()) {
            ...
            List<Integer> groupMembers =  new ArrayList<>(this.getRoom().getGame().getTeams().get(GameTeam.RED));

            for (Integer groupMember : groupMembers) {
                PlayerEntity player = this.getRoom().getEntities().getEntityByPlayerId(groupMember);
                if(player.getPlayer().getEntity().getGameTeam().equals(GameTeam.RED)) { player.teleportToItem(floorItem);} else { return; }
            }
        }
    }
}
