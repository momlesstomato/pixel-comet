...
/**
 * Describes freeze command behavior for the Comet subsystem.
 */
public class FreezeCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if(user.getPlayer().getEntity().canWalk()){
            user.getPlayer().getEntity().setCanWalk(false);
            sendNotif("Usuario congelado", client);
        } else {
            user.getPlayer().getEntity().setCanWalk(true);
            sendNotif("Usuario descongelado", client);
        }
