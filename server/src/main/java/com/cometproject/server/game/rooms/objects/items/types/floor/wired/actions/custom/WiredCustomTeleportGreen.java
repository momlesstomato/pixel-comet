...
public class WiredCustomTeleportGreen extends WiredActionItem {
    ...
    public void onEventComplete(WiredItemEvent event) {
        ...
        for (long itemId : this.getWiredData().getSelectedIds()) {
            ...
            List<Integer> groupMembers =  new ArrayList<>(this.getRoom().getGame().getTeams().get(GameTeam.GREEN));

            for (Integer groupMember : groupMembers) {
                PlayerEntity player = this.getRoom().getEntities().getEntityByPlayerId(groupMember);
                if(player.getPlayer().getEntity().getGameTeam().equals(GameTeam.GREEN)) { player.teleportToItem(floorItem);} else { return; }
            }
        }
    }
}
