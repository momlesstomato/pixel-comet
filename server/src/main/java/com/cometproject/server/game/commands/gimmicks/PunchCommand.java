...
/**
 * Describes punch command behavior for the Comet subsystem.
 */
public class PunchCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (params.length != 1) {
            sendNotif(Locale.getOrDefault("command.puke.none", "\u00bfA qui\u00e9n quieres pegar?"), client);
            return;
        }
        ...
        if (kissedSession.getPlayer().getData().getUsername().equals(client.getPlayer().getData().getUsername())) {
            sendNotif(Locale.getOrDefault("command.puke.himself", "No puedes pegarte a ti mismo."), client);
            return;
        }
        ...
        if (Math.abs(posX - playerX) < 2 && Math.abs(posY - playerY) < 2) {
            String messagePlayer = "* <b>" + client.getPlayer().getEntity().getUsername() + "</b> golpea a <b>" + kissedSession.getPlayer().getEntity().getUsername() + "</b> *";
            client.getPlayer().getEntity().getRoom().getEntities().broadcastMessage(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), messagePlayer, 15));
            client.getPlayer().getEntity().applyEffect(new PlayerEffect(530, 18));
            kissedSession.getPlayer().getEntity().applyEffect(new PlayerEffect(157, 18));
        } else {
