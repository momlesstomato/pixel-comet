...
public class LogQueries {
    ...
    public static void putRecommendation(int playerId, int guideId, int timestamp) {
        ...
        try {
            ...
            preparedStatement = LogDatabaseHelper.prepare("INSERT INTO moderation_recommendations (`player_id`, `guide_id`, `timestamp`) VALUES(?, ?, ?);", sqlConnection);

            preparedStatement.setInt(1, playerId);
            preparedStatement.setInt(2, guideId);
    ...
    public static List<CommandLogEntry> getOpenEvents() {
        ...
        List<CommandLogEntry> openE = new ArrayList<>();

        try {
            sqlConnection = LogDatabaseHelper.getConnection();

            preparedStatement = LogDatabaseHelper.prepare("SELECT `user_id`,`data`,`room_id`, `timestamp` FROM `logs` WHERE `type` = 'COMMAND' AND `data` LIKE '%:eha%' ORDER BY `timestamp` DESC LIMIT " + limit, sqlConnection);


            resultSet = preparedStatement.executeQuery();

            while (resultSet.next()) {
                openE.add(new CommandLogEntry(resultSet.getInt("room_id"), resultSet.getInt("user_id"), resultSet.getString("data"), resultSet.getInt("timestamp")));
            }
        } catch (SQLException e) {
            LogDatabaseHelper.handleSqlException(e);
        } finally {
