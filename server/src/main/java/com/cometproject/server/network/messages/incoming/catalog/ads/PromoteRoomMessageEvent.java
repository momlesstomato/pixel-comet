...
/**
 * Represents the promote room message event published by the network message subsystem.
 */
...
implements Event {
    ...
    public void handle(Session client, MessageEvent msg) throws Exception {
        ...
        final String primaryCurrency = currencyService.currencyCodeForUseCase(CurrencyUseCases.CATALOG_PROMOTION_PRIMARY);
        final String secondaryCurrency = currencyService.currencyCodeForUseCase(CurrencyUseCases.CATALOG_PROMOTION_SECONDARY);
        if (client.getPlayer().getData().getCredits() < totalCostCredits || client.getPlayer().getData().getCurrencyBalance(primaryCurrency) < totalCostPoints || client.getPlayer().getData().getCurrencyBalance(secondaryCurrency) < totalCostActivityPoints) {
        ...
        client.getPlayer().getData().decreaseCurrency(primaryCurrency, totalCostPoints);
        client.getPlayer().getData().decreaseCurrency(secondaryCurrency, totalCostActivityPoints);
