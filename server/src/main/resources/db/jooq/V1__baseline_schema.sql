-- Sanitized for JOOQ DDLDatabase code generation. Derived from db/migration/V1__baseline_schema.sql.
...
CREATE TABLE IF NOT EXISTS `forum_likes` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `type` enum('Comment','Thread') NOT NULL DEFAULT 'Comment',
...
CREATE TABLE IF NOT EXISTS `forum_threads` (
  ...
  `shortstory` varchar(255) DEFAULT NULL,
  `image` varchar(255) DEFAULT NULL,
  `lastcom` varchar(255) NOT NULL,
  `perma` varchar(255) NOT NULL,
  `sub` enum('0','1','2') NOT NULL DEFAULT '0',
  `room` varchar(50) NOT NULL,
  `notificacion` enum('0','1') NOT NULL DEFAULT '1',
...
CREATE TABLE IF NOT EXISTS `items_moodlight` (
  ...
  `preset_1` varchar(500) NOT NULL,
  `preset_2` varchar(500) NOT NULL,
  `preset_3` varchar(500) NOT NULL,
  PRIMARY KEY (`item_id`)
);
...
CREATE TABLE IF NOT EXISTS `pet_data` (
  ...
  `extra_data` varchar(500) NOT NULL,
  PRIMARY KEY (`id`)
);
...
CREATE TABLE IF NOT EXISTS `ranks` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(50) NOT NULL,
  `badgeid` varchar(10) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `uotw` (
  ...
  `userid3` varchar(255) DEFAULT NULL
);
...
CREATE TABLE IF NOT EXISTS `cms_referred` (
  ...
  `referred_ip` varchar(100) NOT NULL,
  `timestamp` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`)
);
